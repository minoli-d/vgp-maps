...
df_prioritized <- df %>%
  mutate(
    priority = case_when(
      ...
      # 2. sampling unusable + centroid available
      (missing_sampling_location_flag | vague_location_flag | outside_range_flag |
         zoo_flag | domesticated_flag) & !missing_range_flag ~ 2L,
      
      # 3. sampling exists but invalid
      (!missing_sampling_location_flag) & 
        (outside_range_flag | vague_location_flag | zoo_flag | domesticated_flag) ~ 3L,
  ...
  select(scientific_name, iucn_name, assessment_id, category, criteria,
...
write.csv(df_prioritized, "results/summary/all_species_prioritized.csv", sep=",")
