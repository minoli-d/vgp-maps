...
#   
#   # zoo should be on the map - for wild captured samples location is valid
#   # if the sample is from zoo - use centroid and put in 6L
#   mutate(
#     priority = case_when(
#       # 1. both sampling and centroid missing
#       # if vague location exists, remove from here
#       missing_sampling_location_flag  & missing_range_flag ~ 1L,
#       
#       # 2. sampling missing, excluding zoo/aquarium
#       # if vague location exists, remove from here
#       missing_sampling_location_flag & !zoo_flag ~ 2L, 
#       
#       # 3. vague sampling that doesn’t overlap range OR no range
#       (vague_location_flag & outside_range_flag) | (vague_location_flag & missing_range_flag) ~ 3L,
#       
#       # 4. precise sampling that doesn’t overlap range
#       # check manually again - cross check w notes/comments
#       (!vague_location_flag & outside_range_flag) ~ 4L, 
#       
#       # 5. vague sampling inside range
#       (vague_location_flag & !outside_range_flag) ~ 5L, 
#       
#       # 6. precise sampling with no range OR precise inside range
#       # add zoo animals in here 
#       # check manually again - cross check w known range of the species - google/wiki
#        (!vague_location_flag & missing_range_flag) | (!vague_location_flag & !outside_range_flag) ~ 6L,
#       
#       TRUE ~ 7L
#     )
#   ) %>%
...
#          accession_number_for_main_haplotype, biosample_accession,
#          comments, geo_location, latlon, lat, lon,
...
df_prioritized <- df %>%
  # filter(!domesticated_flag,
         # !grepl("Homo sapiens", scientific_name)) %>%
  mutate(
    priority = case_when(
      # 1. both sampling & range missing (no vague location)
      missing_sampling_location_flag & missing_range_flag & !vague_location_flag ~ 1L,
      
      # 2. sampling missing, excluding zoo, and not just vague location
      missing_sampling_location_flag & !zoo_flag & !vague_location_flag ~ 2L,
      
      # 3. vague sampling that doesn’t overlap range OR no range
      (vague_location_flag & outside_range_flag) | 
      (vague_location_flag & missing_range_flag) ~ 3L,
      
      # 4. precise sampling that doesn’t overlap range
      (!vague_location_flag & outside_range_flag & !zoo_flag) ~ 4L,
      
      # 5. vague sampling inside range
      (vague_location_flag & !outside_range_flag) ~ 5L,
      
      # 6. precise sampling with no range OR precise inside range OR zoo animals
      (!vague_location_flag & missing_range_flag) |
      (!vague_location_flag & !outside_range_flag) |
      (zoo_flag) ~ 6L,
  ...
  dplyr::select(scientific_name, iucn_name, assessment_id, category, criteria,
         pop_trend, locations, year_published, latest, gbif_synonyms,
         iucn_synonyms, lineage, extended_lineage, english_name,
         accession_number_for_main_haplotype, biosample_accession,
         geo_location, latlon,
         biosample_provider, biosample_collectedby, biosample_comment,
         assembly_comments,
         lat, lon,
         vague_location_flag, missing_sampling_location_flag,
         raster_path_ea, raster_path_goode, centroid_lon, centroid_lat,
...
```{r}
flag_line <- function(row) {
  bits <- c(
    if (isTRUE(row$outside_range_flag) && !isTRUE(row$zoo_flag)) "Outside range",
    if (isTRUE(row$vague_location_flag))                         "Vague location",
    if (isTRUE(row$missing_sampling_location_flag))              "Missing sampling coords",
    if (isTRUE(row$missing_range_flag))                          "Missing range raster",
    if (isTRUE(row$zoo_flag))                                    "Zoo/Aquarium sample",
    if (isTRUE(row$domesticated_flag))                           "Domesticated"
  )
  if (length(bits) == 0) "—" else paste(bits, collapse = " · ")
}

for (i in seq_len(nrow(df_prioritized))) {
  row <- df_prioritized[i, ]

  sp      <- row$scientific_name
  sp_sane <- gsub(" ", "_", sp)
  sp_dir  <- file.path("results/species", sp_sane)
  img_abs <- file.path(sp_dir, paste0(sp_sane, "_distribution_map.png"))
  img_rel <- file.path("..", "species", sp_sane, paste0(sp_sane, "_distribution_map.png"))

  cat("\n\n### ", sp, "\n\n", sep = "")
  cat("**Common Name:** ", ifelse(is.na(row$english_name), "—", row$english_name), "  \n", sep = "")
  cat("**Biosample:** ",    ifelse(is.na(row$biosample_accession) || row$biosample_accession == "", "—", row$biosample_accession), "  \n", sep = "")
  cat("**Assembly Accession:** ", ifelse(is.na(row$accession_number_for_main_haplotype), "—", row$accession_number_for_main_haplotype), "  \n", sep = "")
  cat("**Taxon:** ",        ifelse(is.na(row$lineage), "—", row$lineage), "  \n", sep = "")
  cat("**Priority:** ",     ifelse(is.na(row$priority), "—", row$priority), "  \n", sep = "")
  cat("**Flags:** ",        flag_line(row), "  \n", sep = "")

  if (isTRUE(row$outside_range_flag)) {
    provider  <- ifelse(is.na(row$biosample_provider)  || row$biosample_provider  == "", "—", row$biosample_provider)
    collector <- ifelse(is.na(row$biosample_collectedby) || row$biosample_collectedby == "", "—", row$biosample_collectedby)
    cmt       <- ifelse(is.na(row$biosample_comment) || row$biosample_comment == "", "—", row$biosample_comment)
        loc        <- ifelse(is.na(row$geo_location) || row$geo_location == "", "—", row$geo_location)


    cat("\n**Biosample provider:** ",  provider, "  \n", sep = "")
    cat("**Collected by:** ",          collector, "  \n", sep = "")
    cat("**Biosample comments:** ",    cmt, "  \n", sep = "")
    cat("**Location:** ", loc, "  \n", sep = "")
  }

  if (file.exists(img_abs)) {
    cat("\n![](", img_rel, ")\n", sep = "")
  } else {
    cat("\n_Map image not found._\n")
  }
}
```
