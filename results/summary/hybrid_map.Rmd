...
# europe inset
# euro_bbox <- st_bbox(c(xmin = -30, xmax = 40, ymin = 30, ymax = 70), crs = 4326)
# world_europe <- st_crop(world_ll, euro_bbox)
# euro_bbox_goode <- st_as_sfc(euro_bbox) %>% st_transform(crs_goode)
# location_europe <- st_transform(plot_sf, 4326) %>% st_crop(euro_bbox)

euro_bbox <- st_bbox(c(xmin = -30, xmax = 40, ymin = 25, ymax = 70), crs = 4326)
world_europe <- st_crop(world_ll, euro_bbox)
location_europe <- st_transform(plot_sf, 4326) %>% st_crop(euro_bbox)
...
# n america inset
na_bbox <- st_bbox(c(xmin = -140, xmax = -60, ymin = 0, ymax = 55), crs = 4326)
world_na <- st_crop(world_ll, na_bbox)
location_na <- st_transform(plot_sf, 4326) %>% st_crop(na_bbox)
