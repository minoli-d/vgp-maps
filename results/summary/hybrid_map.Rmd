...
```{r}
df <- read_csv("results/summary/all_species_prioritized.csv", show_col_types = FALSE) %>%
  rename(
    sampling_lon = lon,
    sampling_lat = lat
  ) %>%
...
add_note <- function(tbl, species, note) {
  if (!"map_notes" %in% names(tbl)) tbl <- mutate(tbl, map_notes = NA_character_)
  tbl %>%
    mutate(
      map_notes = if_else(
        .data$scientific_name == species,
        if_else(is.na(.data$map_notes) | .data$map_notes == "",
                note,
                paste(.data$map_notes, note, sep = "; ")),
        .data$map_notes
      )
    )
}

force_centroid <- function(tbl, species, note) {
  add_note(tbl, species, paste0("Manual centroid (", note, ")"))
}

force_sampling <- function(tbl, species, note) {
  add_note(tbl, species, paste0("Manual sampling (", note, ")"))
}
...
set_centroid_coord <- function(df, species, coord_lon, coord_lat, note = NULL) {
  df %>%
    mutate(
      ...
      map_notes = ifelse(scientific_name == species, note, map_notes)
    )
}
...
# species to remove 
remove_species <- c(
  "Branchiostoma floridae",     # lab cross
  "Branchiostoma belcheri",     # lab cross
  "Equus caballus"              # domesticated
)

map_df <- df %>%
  filter(!scientific_name %in% remove_species) %>%
  filter(!domesticated_flag) %>%
  filter(!grepl("Homo sapiens", scientific_name)) %>%
  mutate(map_notes = case_when(
    scientific_name == "Asterias rubens" ~ "Contact submitter: no range or sampling location",
    scientific_name == "Gadus morhua"    ~ "Contact submitter: no range or sampling location",
    TRUE ~ NA
  )) %>%
  ...
  # ----------------------- 
  set_sampling_coord("Cottoperca gobio", 
                     coord_lon=-59.19733, coord_lat=-52.66800,
                     note = "Manual sampling (coords from email)") %>%
  set_sampling_coord("Notothenia rossii", 
                     coord_lon=-62.65000, coord_lat=-63.46667,
                     note = "Manual sampling (coords from email)") %>%
  # ----------------------- 
  force_centroid("Phascolarctos cinereus", "wild sample w vague location - contact submitter") %>% 
  force_centroid("Thunnus thynnus", "sampling loc invalid") %>%
  force_centroid("Macaca fascicularis", "sampling loc invalid") %>%
  # ----------------------- 
  force_sampling("Acridotheres tristis",       "field sample in Tel Aviv") %>%
  force_sampling("Columba livia",              "field sample in Davis") %>%
  force_sampling("Gavia stellata",             "field sample in Greenland") %>%
  force_sampling("Heptranchias perlo",         "field sample in Okinawa") %>%
  force_sampling("Lynx canadensis",            "field sample in Maine") %>%
  force_sampling("Macropus eugenii",           "field sample in Waimangu, NZ") %>%
  force_sampling("Mantella aurantiaca",        "field sample in Madagascar") %>%
  force_sampling("Molossus alvarezi",          "field sample in Lamanai") %>%
  force_sampling("Mops condylurus",            "closer to range than centroid; use sampling") %>%
  force_sampling("Poromitra crassiceps",       "wild pelagic sample") %>%
  force_sampling("Trichomycterus rosablanca",  "field sample in Santander") %>%
  # -----------------------
  ...
  mutate(
    use_coord = case_when(
      ...
      # priorities
      priority == 1 ~ NA,
      priority == 2 ~ "centroid",    # missing all sampling info
      priority == 3 ~ "centroid",    # vague sampling loc    and  outside range
      priority == 4 ~ "centroid",    # precise sampling loc  and  outside range
      priority == 5 ~ "sampling",    # vague sampling loc    and  inside range
      priority == 6 ~ "sampling",    # precise sampling loc  and  inside range
      priority == 7 ~ NA,
...
write.csv(map_df, "results/summary/all_species_map.csv")
```

```{r fig.width=14, fig.height=7}
plot_df <- map_df %>%
  mutate(
    plot_lon = ifelse(use_coord == "sampling", sampling_lon, centroid_lon),
    plot_lat = ifelse(use_coord == "sampling", sampling_lat, centroid_lat),
    shape = ifelse(use_coord == "sampling", "sampling", "centroid")
  ) %>%
  filter(!is.na(plot_lon) & !is.na(plot_lat)) %>%
  mutate(shape = factor(trimws(shape), levels = c("centroid", "sampling"))) 
...
p_global <- ggplot() +
  ...
  geom_sf(data = goode_outline, fill=NA, color="grey85", size=0.5) +
  geom_sf(data = plot_sf, aes(color = extended_lineage, shape = shape), size = 2, alpha = 0.75) +
  scale_color_manual(values = class_colors, name = "Extended Lineage") +
  scale_shape_manual(values = c("sampling" = 1, "centroid" = 2),
                     name = "Coord type",
                     labels = c("sampling" = "Sampling Location", 
...
# europe inset
euro_bbox_ll <- st_bbox(c(xmin = -20, xmax = 40, ymin = 30, ymax = 70), crs = 4326)
euro_bbox_goode <- st_as_sfc(euro_bbox_ll) %>% st_transform(crs_goode)
world_europe <- st_crop(world_goode, euro_bbox_goode)
...
europe_map <- ggplot() +
  ...
  geom_sf(data = location_europe_j,
          aes(color = extended_lineage, shape = shape), size = 1.5) +
  scale_color_manual(values = class_colors, name = "Extended Lineage") +
  scale_shape_manual(values = c("sampling" = 16, "centroid" = 17),
                     name = "Coord type",
                     labels = c("sampling" = "Sampling Location", 
                                "centroid" = "Range Centroid")) +
...
# n america inset
na_bbox_ll <- st_bbox(c(xmin = -140, xmax = -60, ymin = 0, ymax = 55), crs = 4326)
na_bbox_goode <- st_as_sfc(na_bbox_ll) %>% st_transform(crs_goode)
world_na <- st_crop(world_goode, na_bbox_goode)
